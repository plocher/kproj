...
### version
The Jekyll artifact at `_versions/<Project>/<board_rev>.md` representing a snapshot of one release on the SPCoast site. Tied 1:1 to KiCad PCB `${REVISION}` (the `<DESIGN><LETTER>` form, e.g. `3.0B`). Each kproj invocation writes exactly one version entry. The SCH `${REVISION}` (the `<DESIGN>` form, e.g. `3.0`) is recorded as `design_rev` but does not key the version — multiple PCB layout iterations of the same SCH design are distinct versions.
