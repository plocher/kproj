...
- **Lookup**: `common.datasheet_library.read_datasheet_rows(project_dir, inventory) -> (rows, findings)` invokes `jbom -q bom <project_dir> --inventory <path> -f "reference,datasheet,datasheet_name" -o -` (PATH executable, `-m` fallback; global `-q` precedes the `bom` subcommand and suppresses jBOM's guidance diagnostics on stderr, kproj#41) live at publish time (ADR 0010 amends ADR 0003's read-not-invoke for this one narrow, read-only case) and parses structured per-reference `DatasheetRow` rows from stdout. When `inventory` is unset, the invocation is skipped entirely - no subprocess, no finding (kproj#36). `distinct_datasheet_names(rows) -> tuple[str, ...]` derives the case-insensitively deduped, sorted curated names from those rows; `read_datasheet_names` is a thin convenience wrapper composing both for callers that only need the distinct-name shape. Every other failure mode (jBOM missing/old/crashed, absent column) degrades to an advisory `datasheet_field_missing` warning `Finding` rather than raising.
- **URL construction**: `common.datasheet_library.build_datasheet_link(name, *, owner_repo=DEFAULT_DATASHEET_REPO) -> DatasheetLink` is a pure function building the two deterministic, `main`-branch URLs (view: GitHub blob; download: raw.githubusercontent.com) per jBOM#350's URL contract — no commit pinning, relying on the library's Never-Rename invariant. `owner_repo` is overridable (kproj#37's `--datasheet-repo`).
- **Advisory guard**: `common.datasheet_library.check_datasheet_links(names, library_repo) -> findings` is a read-only, never-blocking check against the conventional local datasheet-library clone (`config.DEFAULT_DATASHEET_LIBRARY`, overridable via `PublishWorkflow(library_repo=...)` for tests, or `KprojConfig.datasheet_library` / `--datasheet-library` in production), warning on an unresolvable name or an unpushed library clone.
...
- **Configuration**: `KprojConfig.inventory: Path | None` (CLI `--inventory`/`KPROJ_INVENTORY` env/`~/.kproj.yaml` `inventory:` key precedence, no hardcoded fallback; `None` skips the `jbom bom` invocation entirely, kproj#36) forwards to `jbom bom --inventory` when set. `KprojConfig.datasheet_library: Path` / `datasheet_repo: str` (kproj#37) follow the same CLI/env/yaml tiers but keep SPCoast-convention hardcoded fallbacks (`DEFAULT_DATASHEET_LIBRARY` / `DEFAULT_DATASHEET_REPO`). `kproj --help`'s epilog documents the full precedence chain, every `KPROJ_*` var, and a `~/.kproj.yaml` example; a one-time INFO hint fires on first run when no yaml file exists and `inventory` is unset.
...
## Site-management commands (`list`, `delete`)
...
- `kproj list [project]` reports discovered versions (`*.md` excluding `_index.md`) for a resolved project identifier. When `[project]` is omitted, it defaults to `.` and resolves from CWD using publish-style project resolution.
- `kproj list --all` lists every discovered published project/version shape.
- List output is one line per project in the format: `<project> [<natural-ordered versions>]`.
...
- Log to stderr: pre-flight/runtime status, a compact findings summary (counts by source/severity), and the would-be public destination. The destination derives `baseURL` from `<site_repo>/hugo.toml`; if unavailable, kproj prints the site-relative URL.
...
|---|---|---|
| (none) | WARNING | Runtime/publish status, compact findings summary, and mechanical failures. |
| `-v` / `--verbose` | INFO | Default output plus subprocess (kicad-cli / iBOM / git) argv on every invocation; per-artifact Make-style regen decisions; `production_stale` tolerance suppression (with mtime delta); BOM/POS candidate selection. |
...
Audit / DRC / ERC findings are always preserved in `PublishResult` for version-page tables/front-matter counts and exit-code mapping. Terminal stderr emits an aggregate findings summary by default and under `-v`; detailed per-finding rows via `StderrFormatter` are emitted only when `-d` is set. Logging remains additive to finding/report generation, not a substitute for it.
